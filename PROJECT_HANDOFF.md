...
- **Last updated:** 2026-08-19 13:35 CDT (v0.4.9 added the paper's named tool to the filename; v0.4.9 shipped a regression where the tool consumed the whole keyword budget and dropped every keyword; v0.4.10 fixed it so the name is ADDED to them. Both deployed + live-verified.)
- **Last coding CLI used:** Claude Code CLI (Claude Sonnet 5)
- **Related wiki:** `~/PROJECTS/wiki/concept/hurlab-https-outage` (the recurring HTTPS issue, root-caused + fixed May 2026); `concept/flask-limiter-behind-reverse-proxy` (the rate-limit "Lost connection" gotcha); `concept/browser-folder-upload-recursion-and-limits` (folder upload recursion + per-file vs request size-limit decoupling, from v0.4.5); `concept/llm-structured-extraction-robustness` (coerce-don't-reject schema, deterministic fallbacks for non-deterministic LLMs, prompt-budget, live-log debugging — from v0.4.6→v0.4.8); **new 2026-08-19** — `concept/a-parity-claim-inherits-its-measurement-boundary` (why the v0.4.9 "36/36 byte-identical" parity claim was true of the function and false of the product), `concept/an-added-element-must-not-share-the-existing-budget` (additive-vs-competing in a capped slot; `break` vs `continue` in a budget fill), `concept/probe-the-privilege-envelope-dont-inherit-it` (the stale docker-group line; ownership-not-privilege deploy split; setgid).
...
| **v0.4.9 → v0.4.10 named tool in filename** | **Completed 2026-08-19 13:35 CDT** | 2026-08-19 | Tool/software name (e.g. `MFS-MUnet`) extracted via an optional LLM field + a deterministic title regex, and ADDED to the front of the keywords. v0.4.9 shipped a budget regression (tool alone, no keywords) caught by the user within the session and fixed in v0.4.10. Both deployed + live-verified. |
...
| **Real-PDF confirmation of v0.4.10 (named tool)** | **Open — awaiting user** | 2026-08-19 | v0.4.10 is live and verified in-container on synthetic metadata + the user's exact reported case, but the user has NOT yet re-run real PDFs since the fix. If a paper's tool name is missed, the guards in `extract_tool_name_from_title()` are deliberately conservative — widen the pattern rather than loosening the stop-list. See Session 2026-08-19. |
| **Untracked MoAI template scaffolding** | **Open — needs a keep/drop decision** | 2026-08-19 | A MoAI template refresh (2026-08-19 05:24) left `.github/` (workflows, labels, branch-protection template), `.git_hooks/` (pre-commit, pre-push), `.claudeignore`, `.worktreeinclude` untracked, and rewrote `.gitignore`. The `tmp/` ignore rule it dropped was restored in `5a9907d` (it had re-exposed local research PDFs to `git add`). Still dropped and NOT restored: `!logs/PROJECT_LOG_*.md`, so newly rotated log archives would be silently ignored — the existing archive is already tracked so nothing is lost today. See Session 2026-08-19. |
...
| **Tests for hot paths** | **Open — priority raised 2026-08-19** | 2026-08-19 | Raised from "not started" by the v0.4.9 regression: it would have been caught by ONE golden-file test running the FULL rename pipeline (`build_filename` + `_truncate_keywords`) over a fixture whose keywords contain a multi-word phrase. Every fix v0.4.6→v0.4.10 has instead been verified by ad-hoc standalone scripts, each re-written from scratch. Still uncovered: `LLMService._parse_response` (per-provider shapes incl. reasoning_content fallback), per-file stage updates in `_process_files_background`, and now the filename pipeline end-to-end. Constraint: pytest cannot collect in the dev tree (flask deps live only in the Docker image), so the test job likely has to run in-container. See Session 2026-08-19. |
...
|---|---|---|---|
| Filename length with a tool name | Open (cosmetic) | 2026-08-19 | The tool name is ADDITIVE to the 5-word keyword allowance (that is the v0.4.10 fix), so a paper with one yields a longer filename than before — up to 5 keyword words plus the name. Deliberate; flagged only so a future length complaint is not mis-diagnosed as a regression. |
| Conservative tool-name guards may miss real names | Open (by design) | 2026-08-19 | `extract_tool_name_from_title()` requires an internal capital / all-caps run / digit, rejects a stop-list of article types, and returns `""` rather than guessing — a wrong tool name in a filename is worse than an absent one. Consequence: a genuinely lowercase or prose-styled tool name is missed. Correct response to a miss is to widen the shape test, NOT to loosen the stop-list. |
...
| **v0.4.9 parity claim — CORRECTED** | re-measured end-to-end (both pipeline stages) | The session's earlier "36/36 byte-identical" was measured on `build_filename` ALONE while the pipeline also ran `_truncate_keywords` — true of the function, false of the product. Honest v0.4.9 figure: 28/30. v0.4.10 re-verified end-to-end at 30/30. | 2026-08-19 |
| **Real-PDF end-to-end on v0.4.10** | **NOT YET VERIFIED** — awaiting user upload | In-container checks used synthetic metadata plus the user's exact reported filename; no real PDF has gone through the live app since the fix. | — |
...
- **Resuming:** the next session can simply say **"resume"** — the saved prompt in `NEXT_SESSION.md` (project root, machine-local/untracked; surfaced by the ⭐ pointer in Claude Code auto-memory) drives it. The agent reads it, summarizes in one line, **confirms before executing**, and verifies each precondition mechanically. No paste block needed.
- **Recommended next actions (in priority order):**
  0. **Confirm v0.4.10 on real PDFs (user)** — upload the MFS-MUnet paper and one no-tool paper. Expect `Li-Feng_2026_Pattern-Recognition_MFS-MUnet-<keywords>.pdf` and an unchanged no-tool name. If a tool name is missed, send the filename you got — widen the shape test in `extract_tool_name_from_title()`, do NOT loosen the `_NOT_A_TOOL` stop-list.
  0b. **Decide on the untracked MoAI scaffolding** — `.github/`, `.git_hooks/`, `.claudeignore`, `.worktreeinclude` (keep = one `chore:` commit; drop = delete). Also consider restoring `!logs/PROJECT_LOG_*.md` to `.gitignore`, which the same template refresh dropped.
  ...
  5. **Tests (priority raised)** — the single highest-value test is a golden-file case running the FULL rename pipeline (`build_filename` + `_truncate_keywords`) over metadata whose keywords contain a multi-word phrase; that one fixture would have caught the v0.4.9 regression. Then `PaperMetadata` tolerance, `extract_tool_name_from_title` (incl. the 4 stop-list guards), `extract_pdf_metadata_fields`, `extract_author_from_text`, `_process_files_background`. Constraint: pytest cannot collect in the dev tree — run in-container.
...
- **Last updated:** 2026-08-19 13:35 CDT (v0.4.9 + v0.4.10 named-tool filename component; privileged-ops + deploy instructions corrected; no-paste "resume" path added)
