...
- **Current version:** 0.4.8 (commit `ce24848`, live in production)
- **Last updated:** 2026-06-29 09:02 CDT (extraction-reliability saga v0.4.6→v0.4.8: prompt-budget fix, schema tolerance, deterministic author fallback — user-confirmed working)
...
- **Starting point:** Tip of `main` is commit `ce24848` (v0.4.8). Deploy dir is at the same commit; user confirmed working. Version `0.4.8`, live in production. The extraction-reliability arc (v0.4.6→v0.4.8) is closed.
...
- **Deploy how-to:** edit locally → commit/push to GitHub → run `/home/juhur/tmp/pdf_deploy.sh` on the server (pulls `main` into `/home/hurlab/PROJECTS/research-pdf-renamer` as `hurlab`, `sudo docker build --no-cache --network=host -t ghcr.io/ctr-transcend/research-pdf-renamer:latest .`, `sudo docker compose up -d --force-recreate pdf-renamer`, verifies). The agent can write scripts to the server via `ssh Hurlab '...'` (host key already accepted), but cannot run sudo — the user runs the script. Never use `! sudo`.
- **Last updated:** 2026-06-29 09:02 CDT (extraction reliability v0.4.6→v0.4.8 session)
