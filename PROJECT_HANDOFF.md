...
- **Current version:** 0.4.9 (commit `5a9907d`, live in production)
- **Last updated:** 2026-08-19 (v0.4.9: the paper's named tool/software — e.g. `MFS-MUnet` — now leads the keywords in the filename; deployed + live-verified)
...
- **Starting point:** Tip of `main` is commit `5a9907d` (v0.4.9). Deploy dir + container are at the same commit; version `0.4.9`, live and healthy in production. The extraction-reliability arc (v0.4.6→v0.4.8) is closed; v0.4.9 adds the named-tool filename component on top of it.
