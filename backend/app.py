...
class ApplicationRootMiddleware:
    ...
    def __call__(self, environ, start_response):
        ...
        if 'REQUEST_URI' in environ and self.application_root and self.application_root != '/':
...
def create_app(config_name=None):
    ...
    with app.app_context():
        ...
        if create_admin and admin_password and not User.query.filter_by(email=admin_email).first():
            admin = User(
                email=admin_email,
                name='Administrator',
                is_admin=True,
            ...
            print("[Warning] ADMIN_CREATE is set but ADMIN_PASSWORD is not provided. Skipping admin creation.")
